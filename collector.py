...
def parse_naver_date(raw_date):
    """네이버 API 날짜 파싱. pubDate(RFC 2822) 또는 postdate(YYYYMMDD) 지원."""
    if not raw_date:
        return None
    try:
        # postdate 형식: "20260511"
        if re.match(r"^\d{8}$", raw_date):
            return datetime.strptime(raw_date, "%Y%m%d").replace(tzinfo=KST)
        # pubDate 형식: "Mon, 11 May 2026 12:00:00 +0900"
        from email.utils import parsedate_to_datetime
        return parsedate_to_datetime(raw_date).astimezone(KST)
    except Exception:
        return None
...
def collect_naver(keyword, search_type, keyword_type, max_items):
    ...
    cutoff = datetime.now(KST) - timedelta(days=DAYS_LOOKBACK)
    articles = []
    for item in items:
        title = clean_html(item.get("title", ""))
        desc = clean_html(item.get("description", ""))

        # 날짜 필터: DAYS_LOOKBACK 기간 밖이면 제외
        raw_date = item.get("pubDate") or item.get("postdate")
        pub_dt = parse_naver_date(raw_date)
